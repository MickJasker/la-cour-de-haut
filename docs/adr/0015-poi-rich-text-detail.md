...
- **Route.** `/[locale]/poi/[slug]` is a standalone page (Header + the cached detail read, `generateMetadata` with canonical + hreflang via the same cached slug query) with `generateStaticParams` over published slugs, so it prerenders to static HTML per slug; `@modal/(.)poi/[slug]` intercepts the same URL on soft-nav into a Dialog (`router.back()` on close), reusing the existing `@modal` slot. The server-rendered detail is passed as `children` into the client Dialog wrapper. No `<Suspense>` is needed — the slug query is `"use cache"`, unlike booking's request-time read (ADR-0009). Unknown/unpublished slugs call `notFound()` (ADR-0011). Published POIs are appended to the cached sitemap.
