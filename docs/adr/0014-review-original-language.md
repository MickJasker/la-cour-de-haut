...
- `body[original_locale]` (when the original is one of the four) always mirrors `original_body`: `buildReviewBody` re-seeds it on every save as the single writer, so the source display slot is not an independently editable copy. The owner edits the original text itself; there is no separate "cleaned-up" display slot for the source locale.
