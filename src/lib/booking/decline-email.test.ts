...
const { sendDeclineEmail } = await import("./decline-email");
...
describe("sendDeclineEmail", () => {
  ...
  it("fails loudly when RESEND_API_KEY is not configured (not a silent no-op)", async () => {
    await expect(sendDeclineEmail(baseParams)).rejects.toThrow(
      /RESEND_API_KEY/,
    );
  ...
  it("uses a deterministic no-op stub under E2E_TESTING, even without a configured key", async () => {
    ...
    await expect(sendDeclineEmail(baseParams)).resolves.toBeUndefined();
    expect(mockSend).not.toHaveBeenCalled();
    expect(MockResend).not.toHaveBeenCalled();
  });

  it("sends via Resend, noting the requested dates aren't available", async () => {
    process.env.RESEND_API_KEY = "re_test_key";

    await sendDeclineEmail(baseParams);
    ...
    // Requested arrival date rendered (long-form, en-GB).
    expect(call.html).toContain("1 September 2027");
  });

  it("never mentions refunds, amounts, or bank details", async () => {
    process.env.RESEND_API_KEY = "re_test_key";
  ...
  it("falls back to the English template for an unknown locale", async () => {
    ...
    await sendDeclineEmail({ ...baseParams, locale: "xx" });

    const call = mockSend.mock.calls[0]![0] as { subject: string };
    expect(call.subject).toMatch(/not available|unavailable/i);
  });
  ...
  it.each([
    ...
    "renders the requested date per locale %s",
    async (locale, expectedDate) => {
      process.env.RESEND_API_KEY = "re_test_key";

      await sendDeclineEmail({ ...baseParams, locale });
